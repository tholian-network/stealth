...
const _upgrade = function(socket, nonce, host, port) {
	...
	socket.once('data', (data) => {
		...
		if (
			...
			let expect = crypto.createHmac('sha1').update(nonce.toString('base64') + '258EAFA5-E914-47DA-95CA-C5AB0DC85B11').digest('base64');
			if (accept === expect) {
				socket.emit('upgrade');
			} else {
				socket.emit('error');
...
Client.prototype = Object.assign({}, Emitter.prototype, {
	...
	connect: function(host, port, callback) {
		...
		if (host !== null && port !== null) {
			...
			socket.once('connect', () => _upgrade(socket, nonce, host, port));
