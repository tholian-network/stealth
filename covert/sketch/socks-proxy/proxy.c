...
int resolve_sa (const char *host, unsigned short port, union sockaddr_union *res) {
	...
	if ((ret = resolve(host, port, &ainfo))) {
		return ret;
	}
...
int proxy_waitclient (struct server *server, struct client* client) {
...
int proxy_setup(struct server *server, const char* listenip, unsigned short port) {

	struct addrinfo *ainfo = 0;

	if (resolve(listenip, port, &ainfo)) {
		return -1;
	}
